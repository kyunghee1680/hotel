<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            HotelReservation # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            HotelReservation
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <Number label="RoomNumber" v-model="value.roomNumber" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
...
<script>
    ...
    export default {
        name: 'ReservationHotelReservation',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/hotelReservations'), this.value)
                        } else {
